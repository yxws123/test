...
import com.kline.blconfirm.service.BlMasterAService;
...
@InjectName("blMasterBModule")
// @Filters(@By(type = AuthFilter.class, args = { "admin", "/error/error.jsp"
// }))
@At("/blconfirm/blMasterBAdmin")
@Fail("json")
public class BlMasterBModule {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(BlMasterBModule.class);

	private BlMasterAService blMasterAService;

	@At
	@Ok("jsp:jsp.blconfirm.blMasterBEdit")
	public void editBlMaster(@Param("action") String action,
		...
		if ("create".equals(action)) {
			...
			BlMasterA blMasterA = blMasterAService
					.getById(blMasterId);

			request.setAttribute("blMasterA", blMasterA);
		}
	}

	@At
	@Ok("json")
	public BlMasterA saveBlMaster(HttpServletRequest request,
			@Param("action") String action,
			@Param("blMasterId") long blMasterId) {

		User myAccount = AuthUtil.getMyAccount(request);

		BlMasterA blMasterA = null;

		blMasterA = blMasterAService.getById(blMasterId);

		if (null == blMasterA) {
			// TODO: error processing
		}
		blMasterA.setUpdateInfo(myAccount.getUserId());
//		blMasterA.getBlMasterA().setUpdateInfo(myAccount.getUserId());
...
//		}
		...
		blMasterAService.update(blMasterA);
